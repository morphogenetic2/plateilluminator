document.addEventListener('DOMContentLoaded', () => {
    ...
    const NUM_LEDS = 24;
    const State = {
        programData: {},            // { "LED0": { chunks: [...] }, ... }
        selectedLedIndices: new Set(),
        currentlyViewedLedIndex: null,
        currentChunkIndex: 0,       // Which chunk we're currently editing
        currentStepType: 'ON',
    };

    function initState() {
        for (let i = 0; i < NUM_LEDS; i++) {
            // Initialize with chunk structure
    ...
    function renderTimeline() {
        ...
        if (State.currentlyViewedLedIndex === null || !State.programData[`LED${State.currentlyViewedLedIndex}`]) {
            ...
            noStepsMsg.textContent = (State.currentlyViewedLedIndex !== null)
                ? `LED ${State.currentlyViewedLedIndex + 1} has no chunks yet.`
        ...
        if (chunks.length === 0) {
            const noStepsMsg = document.createElement('p');
            noStepsMsg.className = 'has-text-centered has-text-grey p-4';
            noStepsMsg.textContent = `LED ${State.currentlyViewedLedIndex + 1} has no chunks. Click "Add Chunk" to begin.`;
            timelineDisplayDiv.appendChild(noStepsMsg);
            return;
        }

        // Get current chunk
        if (State.currentChunkIndex >= chunks.length) {
            State.currentChunkIndex = 0;
        }

        const currentChunk = chunks[State.currentChunkIndex];
        const steps = currentChunk.steps || [];
        ...
        steps.forEach((step, originalIndex) => {
            ...
            deleteBtn.addEventListener('click', (e) => {
                ...
                const ledData = State.programData[currentLedKey];
                if (ledData && ledData.chunks && ledData.chunks[State.currentChunkIndex]) {
                    const currentChunk = ledData.chunks[State.currentChunkIndex];
                    currentChunk.steps.splice(originalIndex, 1);
                    renderTimeline();
        ...
        if (steps.length > 0) {
            timelineSortableInstance = new Sortable(timelineWrapper, {
                ...
                onEnd: function (evt) {
                    ...
                    if (ledData && ledData.chunks && ledData.chunks[State.currentChunkIndex]) {
                        const currentChunk = ledData.chunks[State.currentChunkIndex];
                        const [movedItem] = currentChunk.steps.splice(evt.oldIndex, 1);
                        currentChunk.steps.splice(evt.newIndex, 0, movedItem);
                        renderTimeline();
                        document.dispatchEvent(new CustomEvent('programChanged', { detail: { ledIndex: State.currentlyViewedLedIndex } }));
                    }
                }
    ...
    function exportProgram() {
        ...
        if (!hasAnySteps) return alert("The program is empty. Add some chunks and steps before exporting.");
    ...
    function loadProgram(file) {
        ...
        reader.onload = (e) => {
            try {
                ...
                const convertedData = {};
                for (const ledKey in loadedProgram) {
                    const ledData = loadedProgram[ledKey];

                    // Detect format
                    if (Array.isArray(ledData)) {
                        // Legacy format: convert to single chunk
                        console.log(`Converting legacy format for ${ledKey}`);
                        convertedData[ledKey] = {
                            chunks: [
                                {
                                    id: "chunk0",
                                    steps: ledData,
                                    // Legacy programs repeat indefinitely until global timeout
                                    repeat_duration_minutes: null,
                                    repeat_count: null
                                }
                            ]
                        };
                    } else if (ledData && ledData.chunks) {
                        // New chunk format - use as-is
                        convertedData[ledKey] = ledData;
                    } else {
                        // Unknown format - initialize empty
                        convertedData[ledKey] = { chunks: [] };
                    }
                }

                State.programData = convertedData;
                State.currentChunkIndex = 0;
                console.log("Program loaded and converted.");
    ...
    function updateChunkSelector() {
        const chunkSelector = document.getElementById('chunk-selector');
        const chunkLedLabel = document.getElementById('chunk-led-label');
        const chunkConfigSection = document.getElementById('chunk-config-section');

        if (!chunkSelector) return;

        if (State.currentlyViewedLedIndex === null) {
            chunkSelector.innerHTML = '<option>No LED selected</option>';
            chunkLedLabel.textContent = 'No LED Selected';
            chunkConfigSection.style.display = 'none';
            return;
        }

        chunkLedLabel.textContent = `LED ${State.currentlyViewedLedIndex + 1}`;
        const ledData = State.programData[`LED${State.currentlyViewedLedIndex}`];
        const chunks = ledData.chunks || [];

        if (chunks.length === 0) {
            chunkSelector.innerHTML = '<option>No chunks yet</option>';
            chunkConfigSection.style.display = 'none';
            return;
        }
        ...
        chunks.forEach((chunk, index) => {
            ...
            option.value = index;
            const chunkName = chunk.id || `Chunk ${index + 1}`;
            let repeatInfo = '';
            ...
            else repeatInfo = ' (once)';
            option.textContent = chunkName + repeatInfo;
            chunkSelector.appendChild(option);
        });

        chunkSelector.value = State.currentChunkIndex;
        chunkConfigSection.style.display = 'block';
        updateChunkConfigUI();
    }
    ...
    function updateChunkConfigUI() {
        ...
        // Update chunk name input
        document.getElementById('chunk-name-input').value = currentChunk.id || '';

        // Determine repeat mode
        let mode = 'once';
        if (currentChunk.repeat_count !== null && currentChunk.repeat_count !== undefined) {
            mode = 'count';
            document.getElementById('repeat-count-input').value = currentChunk.repeat_count;
        } else if (currentChunk.repeat_duration_minutes !== null && currentChunk.repeat_duration_minutes !== undefined) {
            mode = 'duration';
            document.getElementById('repeat-duration-input').value = currentChunk.repeat_duration_minutes;
        }

        // Update repeat mode buttons
        document.querySelectorAll('.repeat-mode-btn').forEach(btn => {
            btn.classList.remove('is-selected', 'is-info');
        ...
        document.getElementById('repeat-count-field').style.display = mode === 'count' ? 'block' : 'none';
        document.getElementById('repeat-duration-field').style.display = mode === 'duration' ? 'block' : 'none';
    }
    ...
    function saveChunkConfig() {
        ...
        currentChunk.id = chunkName || `chunk${State.currentChunkIndex}`;

        // Determine selected mode
        const selectedMode = document.querySelector('.repeat-mode-btn.is-selected')?.dataset.mode || 'once';

        // Reset all repeat settings
        currentChunk.repeat_count = null;
        currentChunk.repeat_duration_minutes = null;

        // Apply selected mode to ALL selected LEDs
        if (selectedMode === 'count') {
            const count = parseInt(document.getElementById('repeat-count-input').value) || 1;
            const repeat_count = Math.max(1, count);
            State.selectedLedIndices.forEach(ledIdx => {
                const targetLedData = State.programData[`LED${ledIdx}`];
                if (targetLedData && targetLedData.chunks && State.currentChunkIndex < targetLedData.chunks.length) {
                    const chunk = targetLedData.chunks[State.currentChunkIndex];
                    chunk.id = chunkName || `chunk${State.currentChunkIndex}`;
                    chunk.repeat_count = repeat_count;
                    chunk.repeat_duration_minutes = null;
                }
            });
        } else if (selectedMode === 'duration') {
            const duration = parseFloat(document.getElementById('repeat-duration-input').value) || 1;
            const repeat_duration = Math.max(0, duration);
            State.selectedLedIndices.forEach(ledIdx => {
                const targetLedData = State.programData[`LED${ledIdx}`];
                if (targetLedData && targetLedData.chunks && State.currentChunkIndex < targetLedData.chunks.length) {
                    const chunk = targetLedData.chunks[State.currentChunkIndex];
                    chunk.id = chunkName || `chunk${State.currentChunkIndex}`;
                    chunk.repeat_count = null;
                    chunk.repeat_duration_minutes = repeat_duration;
                }
            });
        } else {
            // Mode is 'once' - set both to null
            State.selectedLedIndices.forEach(ledIdx => {
                const targetLedData = State.programData[`LED${ledIdx}`];
                if (targetLedData && targetLedData.chunks && State.currentChunkIndex < targetLedData.chunks.length) {
                    const chunk = targetLedData.chunks[State.currentChunkIndex];
    ...
    function updateLedOutline(ledIndex, allLedButtons) {
        ...
        const ledData = State.programData[`LED${ledIndex}`];
        const hasProgram = ledData && ledData.chunks && ledData.chunks.length > 0;
        if (hasProgram) btn.classList.add('has-program');
    ...
    function initUI() {
        ...
        for (let i = 0; i < NUM_LEDS; i++) {
            ...
            ledButton.addEventListener('click', (event) => {
                ...
                State.currentChunkIndex = 0; // Reset to first chunk when switching LEDs
        ...
        ledActionsContainer.appendChild(createBtn({ icon: 'fa-eraser', text: 'Clear Selected', cls: 'button is-warning control' }, null, () => {
            ...
            if (!confirm(`Clear all chunks and steps for ${State.selectedLedIndices.size} LEDs?`)) return;
        ...
        if (addStepButton) {
            addStepButton.addEventListener('click', () => {
                ...
                // Check if selected LEDs have at least one chunk
                for (const idx of State.selectedLedIndices) {
                    const ledData = State.programData[`LED${idx}`];
                    if (!ledData.chunks || ledData.chunks.length === 0) {
                        return alert(`LED ${idx + 1} has no chunks. Click "Add Chunk" first.`);
                    }
                }
        ...
        // Add Chunk Button
        const addChunkButton = document.getElementById('add-chunk-button');
        if (addChunkButton) {
            addChunkButton.addEventListener('click', () => {
                ...
                // Update current chunk index to the newly added chunk
                if (State.currentlyViewedLedIndex !== null) {
                    const ledData = State.programData[`LED${State.currentlyViewedLedIndex}`];
        ...
        // Remove Chunk Button
        const removeChunkButton = document.getElementById('remove-chunk-button');
        if (removeChunkButton) {
            removeChunkButton.addEventListener('click', () => {
                if (State.currentlyViewedLedIndex === null) return alert("Select an LED first.");

                const ledData = State.programData[`LED${State.currentlyViewedLedIndex}`];
                if (!ledData.chunks || ledData.chunks.length === 0) return alert("No chunks to remove.");
        ...
        // Chunk Selector Change
        const chunkSelector = document.getElementById('chunk-selector');
        if (chunkSelector) {
            chunkSelector.addEventListener('change', (e) => {
        ...
        // Repeat Mode Buttons
        const repeatModeButtons = document.querySelectorAll('.repeat-mode-btn');
        repeatModeButtons.forEach(btn => {
            btn.addEventListener('click', () => {
                const mode = btn.dataset.mode;
                repeatModeButtons.forEach(b => b.classList.remove('is-selected', 'is-info'));
                btn.classList.add('is-selected', 'is-info');
        ...
        // Auto-save chunk config on input changes
        const chunkNameInput = document.getElementById('chunk-name-input');
        const repeatCountInput = document.getElementById('repeat-count-input');
        const repeatDurationInput = document.getElementById('repeat-duration-input');

        if (chunkNameInput) chunkNameInput.addEventListener('blur', saveChunkConfig);
        if (repeatCountInput) repeatCountInput.addEventListener('blur', saveChunkConfig);
        if (repeatDurationInput) repeatDurationInput.addEventListener('blur', saveChunkConfig);
